	...
	config = function()
		...
		local sources = {
			...
			formatting.clang_format.with {
				extra_args = {
					"--style",
					"{BasedOnStyle: Google, IndentWidth: 4, TabWidth: 4, UseTab: Never, ColumnLimit: 0}"
				},
				filetypes = { 'c', 'cpp', 'mm' },
			},
			...
			formatting.prettier.with {
				...
				extra_args = { '--single-quote', '--jsx-single-quote', '--tab-width', '4', '--print-width', '120' }, -- Set tab width to 4
			},
		...
		-- commented to disable format on save
		...
		null_ls.setup {
			...
			--		-- Only setup autocmd if a custom flag is true
		...
		-- Key mapping for formatting selection in visual mode
